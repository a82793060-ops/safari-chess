...
✅ م٢ (MVP، v28): **المسار المُبوّب** — `js/track.js` (TRACK_ORDER مصدر الحقيقة، بوّابة لينة بلا أقفال)،
`Meta.track` (trackStatus/recordCheckpoint، عتبة 70٪)، شاشة «المسار» (تبويب افتراضي) بـ٦ محطات وحالاتها،
محرّك نقطة تحقّق (اختبارات اختيار + رقعة مصغّرة) لكل المحطات المؤلَّفة، ومحطة play (فوز على الهدف).
✅ v32: نقاط تحقّق داخل المسار لِـtactics (٥: شوكة/تثبيت/سيخ) وendgames (٤: مات الرخ/مربّع البيدق/المعارضة/الترقية)
— كل الوضعيات مُتحقَّق منها بـchess.js، وأُزيلت تحويلة puzzles من `enterStation`.
المرجع: `docs/phase2-track-spec.md`. **متبقٍّ:** م٣: تأليف دروس غنيّة (بمساعدة AI).
