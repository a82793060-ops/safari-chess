...
شاشة «المسار» (تبويب افتراضي) بـ٦ محطات وحالاتها وعدّاد نقاط (n/5).
✅ **v33 — محرّك نقطة تحقّق تفاعليّ**: يلعب المتعلّم النقلة على رقعة مصغّرة (نقر مربّعين)، ويُتحقَّق منها مقابل
`solution` بصيغة `from-to[=Q]` + `accept` (يدعم الترقية والتبييت). ٥ نقاط لكل محطة (school/rules/openings/tactics/endgames)،
كلّها مُتحقَّق منها بـchess.js. النموذج: `Meta.completeCheckpoint(station,cpId)` بحالة `{cps:{}}` لكل محطة (توافق خلفيّ مع
bestScore القديم)، عتبة الإكمال ⌈٧٠٪⌉ = ٤/٥. محطة `play`: `PLAY_OBJECTIVES` (٥ أهداف) تُرصد من `Meta.recordPlayResult`
عند نهاية المباراة (سهل=lion فهرس٣، متوسط=fox فهرس٤؛ queenHangedFree ببحث تقريبيّ في history).
المرجع: `docs/phase2-track-spec.md` + جدول محتوى النقاط. **متبقٍّ:** م٣: تأليف دروس غنيّة (بمساعدة AI).
