// ==== الترجمات: عربي / إنجليزي ====
const I18N = {
  ar: {
    title: "بيدق",
    chooseOpponent: "التدريب",
    setupSub: "اهزم كل مستوى لتفتح الذي بعده... حتى تبلغ القمّة!",
    chooseColor: "العب بقطع",
    timeControl: "الوقت",
    white: "الأبيض",
    black: "الأسود",
    random: "عشوائي",
    startGame: "ابدأ المعركة!",
    you: "أنت",
    moves: "النقلات",
    hint: "تلميح",
    undo: "تراجع",
    resign: "استسلام",
    changeBot: "تغيير الخصم",
    rematch: "مباراة أخرى",
    yourTurn: "دورك — هيا العب!",
    botThinking: "يفكر...",
    check: "كش!",
    youWin: "فزت! 🎉",
    youLose: "خسرت المعركة",
    draw: "تعادل",
    winByCheckmate: "كش مات! أحسنت أيها البطل",
    loseByCheckmate: "كش مات عليك... حاول مرة أخرى",
    winByResign: "استسلم خصمك",
    loseByResign: "استسلمت هذه الجولة",
    winByFlag: "انتهى وقت خصمك — الفوز لك!",
    loseByFlag: "انتهى وقتك!",
    drawStalemate: "تعادل بالجمود — لا نقلات متاحة",
    drawRepetition: "تعادل بتكرار الوضعية",
    drawMaterial: "تعادل — لا قطع كافية للفوز",
    draw50: "تعادل بقاعدة الخمسين نقلة",
    confirmResign: "هل تريد الاستسلام فعلا؟",
    confirmLeave: "هل تريد مغادرة المباراة الحالية؟",
    engineLoading: "جاري تحميل محرك Stockfish...",
    engineReady: "المحرك جاهز ✓",
    engineError: "تعذر تحميل المحرك — تأكد من اتصال الإنترنت ثم أعد المحاولة",
    playingAs_w: "تلعب بالأبيض",
    playingAs_b: "تلعب بالأسود",
    level: "مستوى",
    locked: "اهزم المستوى السابق لفتح هذا التحدي",
    // الأطوار
    modeBot: "التدريب",
    modeOnline: "مع صديق",
    modePuzzles: "الألغاز",
    modeShop: "المتجر",
    // الأونلاين
    playWithFriend: "العب مع صديقك",
    onlineExplain: "أنشئ رابط الدعوة وأرسله لصديقك، وسيبدأ اللعب فور انضمامه",
    createLink: "أنشئ رابط الدعوة",
    copy: "نسخ",
    copied: "تم النسخ ✓",
    waitingFriend: "بانتظار انضمام صديقك...",
    connectingToFriend: "جارٍ الاتصال بصديقك...",
    creatingLink: "جارٍ إنشاء الرابط...",
    friend: "صديقك",
    friendTurn: "دور صديقك...",
    friendResigned: "استسلم صديقك — الفوز لك!",
    friendLeft: "انقطع اتصال صديقك",
    friendLeftSub: "يمكنك إنشاء رابط جديد واللعب من جديد",
    connFailed: "تعذر الاتصال — تحقق من الرابط أو اطلب رابطا جديدا",
    connected: "متصل ✓",
    needServer: "اللعب عبر رابط يتطلب فتح اللعبة من موقع وليس من ملف محلي",
    chat: "الدردشة",
    send: "إرسال",
    typeMessage: "اكتب رسالة...",
    watchLink: "رابط المشاهدة",
    watching: "أنت تشاهد المباراة 👁️",
    watcherCount: "متفرج",
    spectateEnded: "انتهت المباراة",
    // تحدي اليوم
    dailyTitle: "تحدي اليوم 🎯",
    dailyDesc: "اهزم {bot} بقطع {color} — مكافأة مضاعفة ⭐×2",
    dailyDone: "أنجزت تحدي اليوم ✓ عد غدا لتحدٍ جديد!",
    dailyPlay: "ابدأ التحدي",
    dailyTag: "تحدي اليوم",
    // الاقتصاد والتقدم
    bananas: "نقطة",
    yourElo: "تقييمك",
    eloChange: "تقييمك: {elo} ({delta})",
    earned: "كسبت {n} ⭐",
    stats: "إحصائياتك",
    statGames: "المباريات",
    statWins: "انتصارات",
    statStreak: "سلسلة الفوز",
    statBestStreak: "أفضل سلسلة",
    statBestWin: "أقوى فوز",
    statFastestMate: "أسرع كش مات",
    // المتجر
    shopTitle: "المتجر 🛍️",
    shopSub: "اكسب النقاط بالفوز",
    shopBoards: "الرقعات",
    shopPieces: "أطقم القطع",
    shopBacks: "الخلفيات",
    settingsTitle: "الإعدادات",
    secBackground: "الخلفية",
    secBoard: "الرقعة",
    secPieces: "القطع",
    close: "إغلاق",
    modeTrack: "المسار",
    trackTitle: "مسار التعلّم",
    trackDone: "أكملت {n} من {total}",
    mastered: "أتقنت",
    recommendedTag: "التالي المُوصى به",
    cpProgress: "سؤال {i} من {n}",
    cpPass: "أتقنت هذه المحطة!",
    cpFail: "لم تجتز بعد — أعد المحاولة",
    cpScore: "نتيجتك: {s}٪",
    cpStep: "نقطة {i} من {n}",
    cpMakeMove: "العب النقلة الصحيحة على الرقعة",
    cpCorrect: "أحسنت!",
    cpSkip: "تخطّي ←",
    cpConcept: "💡 شرح المفاهيم",
    cpRetry: "أعد المحاولة",
    lessonNext: "التالي",
    lessonPrev: "السابق",
    lessonStart: "ابدأ التمارين",
    lessonSkip: "تخطَّ الشرح ←",
    cpExerciseStep: "تمرين {i} من {n}",
    cpGenericWrong: "نقلة قانونية، لكن ليست الأفضل هنا.",
    cpSolutionWas: "الحلّ: {sol}",
    trophyTitle: "غرفة الكؤوس 🏆",
    rankLabel: "الرتب",
    nextRank: "الرتبة التالية",
    maxRank: "أعلى رتبة! 👑",
    points: "{n} نقطة",
    unlockedBy: "يُفتح بـ",
    rankUp: "ترقية",
    buy: "شراء",
    equipped: "مُستخدم ✓",
    use: "استخدام",
    notEnough: "نقاط غير كافية!",
    // الألغاز
    puzzlesTitle: "الألغاز 🧩",
    puzzlesSub: "حل الألغاز واكسب النقاط — سلسلة أيامك: {streak} 🔥",
    puzzleDaily: "لغز اليوم من ليتشيس",
    puzzleDailyLoad: "جارٍ جلب لغز اليوم...",
    puzzleDailyErr: "تعذر جلب لغز اليوم — تحتاج اتصال إنترنت",
    mate1: "كش مات في نقلة",
    mate2: "كش مات في نقلتين",
    tactic: "اكسب قطعة",
    puzzleYourTurn: "دورك: {goal}",
    puzzleCorrect: "أحسنت! ✓",
    puzzleWrong: "ليست هذه... حاول مجددا",
    puzzleSolved: "حللت اللغز! 🎉",
    puzzleShowSolution: "أرني الحل",
    puzzleNext: "اللغز التالي",
    puzzleBack: "عودة للألغاز",
    solved: "محلول ✓",
    // التحليل
    analyze: "تحليل المباراة",
    analyzing: "جارٍ التحليل... {p}%",
    analysisDone: "اكتمل التحليل",
    clsBest: "ممتازة",
    clsGood: "جيدة",
    clsOk: "مقبولة",
    clsMistake: "خطأ",
    clsBlunder: "خطأ فادح",
    bestWas: "الأفضل كان",
    exitAnalysis: "خروج من التحليل",
    coach: "المدرب",
    // المشاركة
    share: "مشاركة النتيجة",
    copyPGN: "نسخ PGN",
    pgnCopied: "نُسخت المباراة ✓",
    // PWA
    installHint: "ثبّت التطبيق على جهازك لتتعلّم وتتدرّب دون إنترنت!",
    // القسم البطولي والحالات الفارغة
    continueJourney: "تابع رحلتك",
    nextFoe: "التالي: المستوى {elo} — {bot}",
    journeyDone: "أتممت كل المستويات! أنت بطل 🏆",
    emptyMoves: "لا نقلات بعد — المباراة في انتظارك",
    emptyChat: "قل مرحبا لصديقك 👋",
    // الأطوار المتغيرة
    variantTitle: "نمط اللعب",
    variant_standard: "قياسي",
    variant_koth: "ملك التلة ⛰️",
    "variant_3check": "ثلاث كشات ⚔️",
    winKoth: "بلغ ملكك قمة التلة — الفوز لك!",
    loseKoth: "بلغ ملك الخصم قمة التلة",
    win3check: "ثلاث كشات — الفوز لك!",
    lose3check: "تلقيت ثلاث كشات",
    // الأوسمة
    newBadge: "وسام جديد",
    badgesTitle: "الأوسمة",
    // سلسلة الألغاز
    rushTitle: "سلسلة الألغاز 🔥",
    rushDesc: "ألغاز حية بلا حدود تزداد صعوبة — خطأ واحد ينهي السلسلة! أفضلك: {best}",
    rushStart: "ابدأ السلسلة",
    rushProgress: "السلسلة: {n}",
    rushEnd: "انتهت السلسلة عند {n}",
    // التحليل
    accuracy: "الدقة",
    evalGraph: "منحنى المباراة",
    // شارة الدور
    turnYou: "دورك",
    turnThink: "يفكر…",
    turnFriend: "دوره",
    // تدريب المواضيع (lichess)
    themesTitle: "تدريب بالمواضيع",
    themesSub: "ألغاز حقيقية بلا حدود من lichess — اختر الموضوع والصعوبة",
    th_mix: "مزيج تكتيكي",
    th_mateIn1: "كش مات في نقلة",
    th_mateIn2: "كش مات في نقلتين",
    th_fork: "الشوكة",
    th_pin: "التثبيت",
    th_skewer: "السيخ",
    th_hangingPiece: "القطعة السائبة",
    th_sacrifice: "التضحية",
    th_endgame: "النهايات",
    diff_easier: "سهل",
    diff_normal: "متوسط",
    diff_harder: "صعب",
    puzzleFetching: "جارٍ جلب لغز جديد...",
    puzzleFetchErr: "تعذر الجلب — تحقق من اتصال الإنترنت",
    puzzleRating: "صعوبة اللغز",
    // مدرب الافتتاحيات
    openingsTitle: "مدرب الافتتاحيات",
    openingsSub: "تعلم أشهر الافتتاحيات نقلة نقلة — العب نقلات خطتك والمدرب يرد عليك",
    asWhite: "بالأبيض",
    asBlack: "بالأسود",
    openingLearned: "أتقنت الخط الرئيسي! 📖",
    // سجل المباريات
    historyTitle: "آخر المباريات 📜",
    historyEmpty: "لا مباريات بعد — ابدأ رحلتك!",
    // المدرسة
    schoolTitle: "المدرسة 🎓",
    schoolSub: "أول مرة تلعب؟ تعلم حركة كل قطعة بالتقاط البيادق",
    schoolStage: "المرحلة {a} من {b}",
    schoolGoalEat: "التقط كل البيادق السوداء — تبقى {n} 🍽️",
    schoolGoalPromote: "سر بالبيدق إلى الصف الأخير ليترقى وزيرا!",
    schoolGoalCastle: "بيّت ملكك: حرك الملك خطوتين نحو القلعة",
    schoolGoalEp: "بيدق الخصم اندفع خطوتين للتو — التقطه في المرور!",
    schoolGoalCheck: "اصنع كشا لملك الخصم!",
    schoolGoalMate: "كش مات في نقلة واحدة — ابحث عنها!",
    schoolTryAgain: "ليست النقلة المطلوبة — نبدأ من جديد",
    schoolStageDone: "أحسنت! المرحلة التالية...",
    schoolDone: "أتقنت درس {name}! 🎓",
    schoolNext: "الدرس التالي",
    // مدرب النهايات
    egTitle: "مدرب النهايات",
    egSub: "أتقن الحسم — المحرك يدافع بأقصى قوة وعليك إنهاء المهمة دون تعادل",
    egNext: "النهاية التالية",
    drillGoal: "كش مات الملك الأعزل — {name}",
    drillDone: "أتقنت النهاية! 🏁",
    drillFail: "أفلت الملك بالتعادل! لا تدعه يهرب في المرة القادمة",
    drillRetry: "إعادة المحاولة",
    // لوحة التحليل الحرة
    labTitle: "لوحة التحليل الحرة 🔬",
    labDesc: "رتب أي وضعية بيدك، شغّل المحرك عليها، أو العبها ضد بوت",
    labStart: "افتح اللوحة",
    labHint: "اختر قطعة من اللوح ثم المس الرقعة لوضعها — الممحاة تزيل",
    labTurn: "الدور: {side}",
    labStartPos: "وضعية البداية",
    labClear: "إفراغ الرقعة",
    labAnalyze: "حلل الوضعية",
    labPlay: "العب من هنا",
    labBack: "عودة",
    labAnalyzing: "المحرك يفكر... 🔬",
    labKings: "تحتاج ملكا واحدا بالضبط لكل جانب",
    labPawns: "لا بيادق في الصفين الأول والأخير",
    labCheckTurn: "الملك المقابل في كش — بدّل الدور أولا",
    labNoMoves: "الوضعية منتهية أصلا — كش مات أو تعادل",
    labEval: "تقييم الأبيض: {s} — السهم يريك أفضل نقلة",
    labMate: "كش مات خلال {n} لصالح {side}",
    // تدريب الإحداثيات
    coordsTitle: "تدريب الإحداثيات 🧭",
    coordsDesc: "المس المربع المطلوب بأسرع ما يمكن خلال 30 ثانية — أفضل نتيجة: {best}",
    coordsStart: "ابدأ التدريب",
    coordsScore: "النتيجة: {n} — الوقت: {s}",
    coordsEnd: "انتهى الوقت! أصبت {n} مربعا",
    coordsBest: "رقم قياسي جديد! 🎉",
    // التذييل
    footerPuzzles: "الألغاز بفضل lichess.org",
  },
  en: {
    title: "Baydaq",
    chooseOpponent: "Practice",
    setupSub: "Beat each level to unlock the next... all the way to the top!",
    chooseColor: "Play as",
    timeControl: "Time",
    white: "White",
    black: "Black",
    random: "Random",
    startGame: "Start the battle!",
    you: "You",
    moves: "Moves",
    hint: "Hint",
    undo: "Undo",
    resign: "Resign",
    changeBot: "Change opponent",
    rematch: "Rematch",
    yourTurn: "Your turn — go!",
    botThinking: "thinking...",
    check: "Check!",
    youWin: "You win! 🎉",
    youLose: "You lost the battle",
    draw: "Draw",
    winByCheckmate: "Checkmate! Well played, champion",
    loseByCheckmate: "Checkmated... try again",
    winByResign: "Your opponent resigned",
    loseByResign: "You resigned this round",
    winByFlag: "Your opponent ran out of time — you win!",
    loseByFlag: "You ran out of time!",
    drawStalemate: "Draw by stalemate — no moves available",
    drawRepetition: "Draw by repetition",
    drawMaterial: "Draw — insufficient material",
    draw50: "Draw by the fifty-move rule",
    confirmResign: "Do you really want to resign?",
    confirmLeave: "Leave the current game?",
    engineLoading: "Loading Stockfish engine...",
    engineReady: "Engine ready ✓",
    engineError: "Couldn't load the engine — check your internet connection and retry",
    playingAs_w: "Playing as White",
    playingAs_b: "Playing as Black",
    level: "Level",
    locked: "Beat the previous level to unlock this challenge",
    modeBot: "Practice",
    modeOnline: "With a friend",
    modePuzzles: "Puzzles",
    modeShop: "Shop",
    playWithFriend: "Play with your friend",
    onlineExplain: "Create an invite link and send it to your friend — the game starts when they join",
    createLink: "Create invite link",
    copy: "Copy",
    copied: "Copied ✓",
    waitingFriend: "Waiting for your friend to join...",
    connectingToFriend: "Connecting to your friend...",
    creatingLink: "Creating the link...",
    friend: "Your friend",
    friendTurn: "Your friend's turn...",
    friendResigned: "Your friend resigned — you win!",
    friendLeft: "Your friend disconnected",
    friendLeftSub: "You can create a new link and play again",
    connFailed: "Connection failed — check the link or ask for a new one",
    connected: "Connected ✓",
    needServer: "Playing via link requires opening the game from a website, not a local file",
    chat: "Chat",
    send: "Send",
    typeMessage: "Type a message...",
    watchLink: "Spectate link",
    watching: "You are spectating 👁️",
    watcherCount: "spectator(s)",
    spectateEnded: "The game has ended",
    dailyTitle: "Daily challenge 🎯",
    dailyDesc: "Beat {bot} playing {color} — double reward ⭐×2",
    dailyDone: "Daily challenge done ✓ come back tomorrow!",
    dailyPlay: "Start challenge",
    dailyTag: "Daily challenge",
    bananas: "point",
    yourElo: "Your rating",
    eloChange: "Your rating: {elo} ({delta})",
    earned: "You earned {n} ⭐",
    stats: "Your stats",
    statGames: "Games",
    statWins: "Wins",
    statStreak: "Win streak",
    statBestStreak: "Best streak",
    statBestWin: "Strongest win",
    statFastestMate: "Fastest mate",
    shopTitle: "Shop 🛍️",
    shopSub: "Earn points by winning and buy what you like",
    shopBoards: "Boards",
    shopPieces: "Piece sets",
    shopBacks: "Backgrounds",
    settingsTitle: "Settings",
    secBackground: "Background",
    secBoard: "Board",
    secPieces: "Pieces",
    close: "Close",
    modeTrack: "Path",
    trackTitle: "Learning Path",
    trackDone: "Completed {n} of {total}",
    mastered: "Mastered",
    recommendedTag: "Recommended next",
    cpProgress: "Question {i} of {n}",
    cpPass: "Station mastered!",
    cpFail: "Not passed yet — try again",
    cpScore: "Your score: {s}%",
    cpStep: "Move {i} of {n}",
    cpMakeMove: "Play the correct move on the board",
    cpCorrect: "Well done!",
    cpSkip: "Skip →",
    cpConcept: "💡 Concepts",
    cpRetry: "try again",
    lessonNext: "Next",
    lessonPrev: "Previous",
    lessonStart: "Start exercises",
    lessonSkip: "Skip explanation →",
    cpExerciseStep: "Exercise {i} of {n}",
    cpGenericWrong: "Legal move, but not the best one here.",
    cpSolutionWas: "Solution: {sol}",
    trophyTitle: "Trophy Room 🏆",
    rankLabel: "Ranks",
    nextRank: "Next rank",
    maxRank: "Max rank! 👑",
    points: "{n} points",
    unlockedBy: "Unlocked by",
    rankUp: "Rank up",
    buy: "Buy",
    equipped: "Equipped ✓",
    use: "Use",
    notEnough: "Not enough points!",
    puzzlesTitle: "Puzzles 🧩",
    puzzlesSub: "Solve puzzles and earn points — day streak: {streak} 🔥",
    puzzleDaily: "Lichess daily puzzle",
    puzzleDailyLoad: "Fetching today's puzzle...",
    puzzleDailyErr: "Couldn't fetch the daily puzzle — internet needed",
    mate1: "Mate in one",
    mate2: "Mate in two",
    tactic: "Win material",
    puzzleYourTurn: "Your turn: {goal}",
    puzzleCorrect: "Correct! ✓",
    puzzleWrong: "Not that one... try again",
    puzzleSolved: "Puzzle solved! 🎉",
    puzzleShowSolution: "Show solution",
    puzzleNext: "Next puzzle",
    puzzleBack: "Back to puzzles",
    solved: "Solved ✓",
    analyze: "Analyze game",
    analyzing: "Analyzing... {p}%",
    analysisDone: "Analysis complete",
    clsBest: "Excellent",
    clsGood: "Good",
    clsOk: "Okay",
    clsMistake: "Mistake",
    clsBlunder: "Blunder",
    bestWas: "Best was",
    exitAnalysis: "Exit analysis",
    coach: "Coach",
    share: "Share result",
    copyPGN: "Copy PGN",
    pgnCopied: "Game copied ✓",
    installHint: "Install the app to learn and practice offline!",
    continueJourney: "Continue journey",
    nextFoe: "Next: Level {elo} — {bot}",
    journeyDone: "Journey complete! You are a champion 🏆",
    emptyMoves: "No moves yet — the battle awaits you",
    emptyChat: "Say hi to your friend 👋",
    variantTitle: "Game mode",
    variant_standard: "Standard",
    variant_koth: "King of the Hill ⛰️",
    "variant_3check": "Three-check ⚔️",
    winKoth: "Your king reached the hilltop — you win!",
    loseKoth: "The enemy king reached the hilltop",
    win3check: "Three checks — you win!",
    lose3check: "You received three checks",
    newBadge: "New badge",
    badgesTitle: "Badges",
    rushTitle: "Puzzle Streak 🔥",
    rushDesc: "Unlimited live puzzles, harder as you go — one mistake ends the streak! Your best: {best}",
    rushStart: "Start streak",
    rushProgress: "Streak: {n}",
    rushEnd: "Streak ended at {n}",
    accuracy: "Accuracy",
    evalGraph: "Game graph",
    turnYou: "Your turn",
    turnThink: "thinking…",
    turnFriend: "Their turn",
    themesTitle: "Themed training",
    themesSub: "Unlimited real puzzles from lichess — pick a theme and difficulty",
    th_mix: "Tactical mix",
    th_mateIn1: "Mate in one",
    th_mateIn2: "Mate in two",
    th_fork: "Fork",
    th_pin: "Pin",
    th_skewer: "Skewer",
    th_hangingPiece: "Hanging piece",
    th_sacrifice: "Sacrifice",
    th_endgame: "Endgames",
    diff_easier: "Easy",
    diff_normal: "Medium",
    diff_harder: "Hard",
    puzzleFetching: "Fetching a new puzzle...",
    puzzleFetchErr: "Fetch failed — check your internet connection",
    puzzleRating: "Puzzle rating",
    openingsTitle: "Opening Trainer",
    openingsSub: "Learn the classic openings move by move — play your side, the trainer replies",
    asWhite: "as White",
    asBlack: "as Black",
    openingLearned: "Main line mastered! 📖",
    historyTitle: "Recent games 📜",
    historyEmpty: "No games yet — start your journey!",
    schoolTitle: "School 🎓",
    schoolSub: "First time playing? Learn how every piece moves by capturing pawns",
    schoolStage: "Stage {a} of {b}",
    schoolGoalEat: "Capture all the black pawns — {n} left 🍽️",
    schoolGoalPromote: "March your pawn to the last rank to become a queen!",
    schoolGoalCastle: "Castle: move your king two squares toward the rook",
    schoolGoalEp: "Their pawn just rushed two squares past you — capture it en passant!",
    schoolGoalCheck: "Give the enemy king a check!",
    schoolGoalMate: "Mate in one — find it!",
    schoolTryAgain: "Not the move we need — let's restart",
    schoolStageDone: "Well done! Next stage...",
    schoolDone: "You mastered {name}! 🎓",
    schoolNext: "Next lesson",
    egTitle: "Endgame trainer",
    egSub: "Master the finish — the engine defends at full strength; close it out without a draw",
    egNext: "Next endgame",
    drillGoal: "Checkmate the lone king — {name}",
    drillDone: "Endgame mastered! 🏁",
    drillFail: "The king escaped with a draw! Don't let him slip next time",
    drillRetry: "Try again",
    labTitle: "Free analysis board 🔬",
    labDesc: "Set up any position by hand, run the engine on it, or play it vs a bot",
    labStart: "Open the board",
    labHint: "Pick a piece from the palette, then tap the board to place it — the eraser removes",
    labTurn: "To move: {side}",
    labStartPos: "Starting position",
    labClear: "Clear board",
    labAnalyze: "Analyze",
    labPlay: "Play from here",
    labBack: "Back",
    labAnalyzing: "Engine thinking... 🔬",
    labKings: "You need exactly one king per side",
    labPawns: "No pawns on the first or last rank",
    labCheckTurn: "The other king is in check — switch the turn first",
    labNoMoves: "The position is already over — mate or draw",
    labEval: "White's eval: {s} — the arrow shows the best move",
    labMate: "Mate in {n} for {side}",
    coordsTitle: "Coordinate trainer 🧭",
    coordsDesc: "Tap the named square as fast as you can in 30 seconds — your best: {best}",
    coordsStart: "Start training",
    coordsScore: "Score: {n} — time: {s}",
    coordsEnd: "Time's up! You hit {n} squares",
    coordsBest: "New personal best! 🎉",
    footerPuzzles: "Puzzles powered by lichess.org",
  }
};

let LANG = localStorage.getItem("safari-lang") || "ar";

function t(key, vars) {
  let s = (I18N[LANG] && I18N[LANG][key]) || I18N.ar[key] || key;
  if (vars) for (const [k, v] of Object.entries(vars)) s = s.replaceAll(`{${k}}`, v);
  return s;
}

function applyLang() {
  document.documentElement.lang = LANG;
  document.documentElement.dir = LANG === "ar" ? "rtl" : "ltr";
  document.querySelectorAll("[data-i18n]").forEach(el => {
    el.textContent = t(el.dataset.i18n);
  });
  const btn = document.getElementById("btn-lang");
  if (btn) btn.textContent = LANG === "ar" ? "EN" : "ع";
  document.title = LANG === "ar" ? "بيدق ♟" : "Baydaq ♟";
}

function toggleLang() {
  LANG = LANG === "ar" ? "en" : "ar";
  localStorage.setItem("safari-lang", LANG);
  applyLang();
  document.dispatchEvent(new CustomEvent("langchange"));
}
